    ...
    const std::wstring& core_clr_directory, dnx::trace_writer& trace_writer)
{
    ...
    trusted_platform_assemblies.append(dnx::utils::path_combine(runtime_directory, L"Microsoft.Dnx.DomainManager.CoreClr.dll"));
    ...
    HRESULT hr = pCLRRuntimeHost->CreateAppDomainWithManager(
        L"Microsoft.Dnx.DomainManager.CoreClr",
    ...
    hr = pCLRRuntimeHost->CreateDelegate(domainId, L"Microsoft.Dnx.DomainManager.CoreClr, Version=0.0.0.0", L"DomainManager", L"Execute", (INT_PTR*)&main_function);
    if (FAILED(hr))
    {
